...
PROJECT=/data/binder_software/BindCraft/runs/15delt3/pre-binder
TARGET=/data/binder_software/BindCraft/runs/15delt3/trial_6_may_20/1lp3_hexamer_trimmed_fixed.pdb
OUTDIR="$PROJECT/outputs/01_rfdiffusion_pilot"
LOGDIR="$PROJECT/logs"
RFD_PY=/data/rfdiffusion/.venv-af2/bin/python
...
EXISTING=$(ls "$OUTDIR"/design_*.pdb 2>/dev/null | wc -l)
...
# Post-flight check
N=$(ls "$OUTDIR"/design_*.pdb 2>/dev/null | wc -l)
echo
