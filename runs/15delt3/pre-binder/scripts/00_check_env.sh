...
echo
echo "=== Inputs ==="
TARGET_PDB=/data/binder_software/BindCraft/runs/15delt3/trial_6_may_20/1lp3_hexamer_trimmed_fixed.pdb
...
if [ -f "$TARGET_PDB" ]; then
  ...
  RES=$(awk '/^ATOM/ && substr($0,22,1)=="A"{r[substr($0,23,4)+0]=1} END{n=0; min=99999; max=0; for(k in r){n++; if(k<min)min=k; if(k>max)max=k} printf "%d residues, range %d-%d", n, min, max}' "$TARGET_PDB")
  say "  Chain A" "$RES"
fi
